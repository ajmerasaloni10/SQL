...
with dep_employee_rank as (SELECT e.Name,e.Salary, d.Name as department_name,
        rank() over( partition by d.name order by Salary desc) rank_n
FROM employee e
INNER JOIN department d 
on e.DepartmentId = d.Id) 

select Name, Salary, department_name
from dep_employee_rank
where rank_n = 1